...
#         print(msg)
#     else:
#         # 💡 Send test message every run
#         msg = f"ℹ️ No cross on {symbol} ({cfg['interval']}) — test alert at {datetime.utcnow().isoformat()}Z"
#         send_telegram(msg)
#         print(msg)
...
def check_symbol(symbol):
    ...
    if ema9_now > ema26_now and ema9_prev <= ema26_prev:
        ...
        # Send test message every run for debug
        msg = f"ℹ️ No cross on {symbol} ({cfg['interval']}) — test alert at {datetime.utcnow().isoformat()}Z"
        send_telegram(msg)
        print(msg)
...
if __name__ == "__main__":
    main()
    # 💡 Force a single test message to confirm Telegram works
    send_telegram("✅ TEST: EMA bot is running and Telegram messages are working!")
